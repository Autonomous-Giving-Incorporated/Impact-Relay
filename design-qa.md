# Design QA — Impact Relay

- Reference: supplied AGI brand board (`codex-clipboard-7b0a155b-424e-4f90-a4c2-7af6e0bd2174.png`)
- Suite contract: [`docs/AGI-DESIGN-SYSTEM.md`](docs/AGI-DESIGN-SYSTEM.md) (must remain coherent with AGI + Fund Intel)
- Implementation: `http://127.0.0.1:8082/index.html`
- Viewport: 1280 × 720
- Comparison artifact: `/private/tmp/impact-qa.png`

## Review

- P0: none
- P1: none
- P2: none
- Identity: AGI is the persistent master brand, Impact Relay is the product, and Hacker Dojo remains campaign context.
- Tokens and type: the evidence-led interface uses the shared AGI palette, Space Grotesk display, and Inter interface typography (aligned with AGI `tokens.css` and Fund Intel shell).
- Navigation: reciprocal product links use the `autogive.app` route family (AGI, Fund Intel, Impact Relay).
- Builder attribution: Zero State appears only in the legal footer beside Tokens, Logo use, and Legal.

final result: passed
